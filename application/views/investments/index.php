...
if ($stat1=='' || ($stat1!='' && $stat1==strtoupper($investment['loanStatusCode'])) || ($stat1==-1 && $investment['loanStatusCode']!=4 && $investment['loanStatusCode']!=8) ) {
	//echo "stat1=".$stat1."   statusCode=".strtoupper($investment['loanStatusCode']);
} else {
	//echo "valitud riik=".$riik1."   tegelik=".$investment['country']."<br>";
	continue;
}
...
*/	
	...
	?>
						<?php
						$makstaveel=$makstaveel+$investment['principalRemaining']; //palju maksta veel
						$interestsum=$interestsum+$investment['interestRepaid'];  //palju on makstud intresse
						if ($investment['listedOnDate']!=NULL) {  //müügisolevate summa ja reaalne väärtus(lahutades allahindluse  %)
							$myygis=$myygis+$investment['principalRemaining'];
							$myygisreaalne=$myygisreaalne+$investment['principalRemaining']+($investment['principalRemaining']*$investment['desiredDiscountRate']/100);
							
						}
						if ($investment['loanStatusCode']==5) { //60+ pļæ½eva viivises
							$summa=$summa+$investment['amount'];
							$default=$default+$investment['amount']-$investment['principalRepaid']-$investment['interestRepaid']; //sama mis järgmine arvutatuna
							$default1=$default1+$investment['principalRemaining']; //kui natuke on maksnud, siis kogu maksmata summa
							$vvv=$vvv+$investment['interestLateAmount'];  //hilinenud intressid
							$vvv1=$vvv1+$investment['interestRepaid'];  //makstud intressid
							$kkk=$kkk+$investment['principalLateAmount'];  //maksmata põhiosa
							$kkk1=$kkk1+$investment['lateAmountTotal'];
							...
							$viivises1=$viivises1+$investment['principalRemaining'];  //palju maksmata, järgi põhiosa
							$summa=$summa+$investment['amount'];
...
<?php //		<td><?php echo $investment['loanId'] ></td>?>
			...
			<table class="table table-bordered table-striped report2" id="report2">
				<tbody>
					...
					<tr>
						...
						<td style="white-space: nowrap;"><?php echo substr($investment['lastPaymentDate'],0,10);?></td>
						<th style="text-align: right; white-space: nowrap;">#LoanStatusActiveFrom</th>
						<td style="white-space: nowrap;"><?php echo str_replace("T"," ",substr($investment['loanStatusActiveFrom'],0,19));?></td>
					</tr>
	...
	<tr>
		...
		<th colspan="999">Müügis: <?php echo $myygis;?> | Müügis kätte: <?php echo $myygisreaalne?> | 60+päeva: <?php echo $default1?> | 60+hilinenud intress: <?php echo $vvv;?></th>

	</tr>
	<tr>
	
		<th colspan="999">Maksta veel: <?php echo $makstaveel;?> | Kõik intressid: <?php echo $interestsum?> | 60+päeva: <?php echo $default1?> | 60+intressi saadud: <?php echo $vvv1;?></th>
