...
class Filters extends CI_Controller {
        ...
        {
                $data['filters_item'] = $this->filters_model->get_filters($id);
                
                if (empty($data['filters_item']))
                {
                	show_404();
                }
                
                $data['title'] = $data['filters_item']['user'];
                
                
                $this->load->view('templates/header', $data);
                $this->load->view('filters/view', $data);
        ...
        {	//echo "del=".$del;
        	...
        	if ($del==2) {
 		       	$this->filters_model->del_filters($id);
			...
			$this->session->set_flashdata('redirect', 'filters/delete/'.$id."/2");
			$this->session->set_flashdata('redirect_msg', 'Kustuta filter');
			$this->session->set_flashdata('redirect_back', 'filters/index');
			...
			$this->load->view('templates/header', $data);
			$this->load->view('filters/success', $data);
			$this->load->view('templates/footer');
				
			} else redirect('filters/index');
        ...
        public function index($token = NULL)
        {
        	...
        	$data['filters'] = $this->filters_model->get_filters();
        	$data['title'] = 'Bondoora ajutised ja m��ratud otsingud';
        	$data['token'] = $token;
        ...
        public function edit($id = NULL)
        {
        	...
        	if ($id == 0)
        		...
        		if ($this->input->post('terms')) {
        			if (!$this->input->post('terms3'))
        				$_POST['terms3'] = 0;
       				if (!$this->input->post('terms9'))
       					$_POST['terms9'] = 0;
   					if (!$this->input->post('terms12'))
   						$_POST['terms12'] = 0;
					if (!$this->input->post('terms18'))
						$_POST['terms18'] = 0;
					if (!$this->input->post('terms24'))
						$_POST['terms24'] = 0;
    				if (!$this->input->post('terms36'))
    					$_POST['terms36'] = 0;
    				if (!$this->input->post('terms48'))
    					$_POST['terms48'] = 0;
    				if (!$this->input->post('terms60'))
    					$_POST['terms50'] = 0;
        	...
        	if ($this->form_validation->run() === FALSE)
        	{
        		...
        		$this->load->view('templates/header', $data);
        		//if ($id == NULL ) 
        		$data=	$this->load->view('filters/edit', $data);
        		//else 
        	...
        	else
        	{
        		...
        		if( $formSubmit == 'formBack' )
        			...
        			$arv=$this->filters_model->set_filters($id);
        			$this->session->set_flashdata('msg', '<div class="alert alert-success text-center">Filter on muudetud!  Muudetud ridasid='.$arv.'</div>');
        			$this->session->set_flashdata('redirect', 'filters/index');
        			$this->session->set_flashdata('redirect_msg', 'Tagasi filtrite nimekirja');
        			$dat['arv'] = $arv;
        			//$this->load->view('filters/success/',$dat);
        			redirect('filters/success/' . $arv);
        		}
        ...
        public function create($id = NULL)
        {
        	...
        	//	$data['title'] = 'Muudame bondoora filtrit';
        			...
        			if ($this->form_validation->run() === FALSE)
        			{
        				...
        				$this->load->view('templates/header', $data);
        				//if ($id == NULL )
        				$this->load->view('filters/create', $data);
        				//else
        				//	$this->load->view(anchor("filters/edit,"filters/edit/".$id), $data);
        				$this->load->view('templates/footer');
        			...
        			else
        			{
        				...
        				$this->load->view('filters/success');
        			}
        }
